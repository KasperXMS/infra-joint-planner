"""Shape all worker paths locally and run one 4090-hosted open-ended cell."""
...
RUN_ID = re.compile(
    r"^[0-9]{2}-[a-z0-9-]+-H_(?:low|mid|high)-infra-aware-open$"
)
...
async def _snapshot(
    ...
    command = (
        ("sudo", "-n", "tc", "qdisc", "show", "dev")
        if kind == "qdisc"
        else ("sudo", "-n", "tc", "-s", "class", "show", "dev")
    )
    values = await asyncio.gather(
        *(
            runner.run(item.ssh_target, (*command, item.interface))
            for item in endpoints
        )
    )
    return {
...
async def run(args: argparse.Namespace) -> None:
    ...
    regime = TcRegime(
        configuration_id=f"open-infra-v1-{args.run_id}",
    ...
    record["original_qdisc"] = await _snapshot(runner, endpoints, "qdisc")
    remote_returncode = 1
    try:
        ...
        record["class_before_run"] = await _snapshot(runner, endpoints, "class")
        remote_command = shlex.join(("cd", args.remote_app)) + " && " + shlex.join(
            (
                "env",
                f"PYTHONPATH={args.remote_app}/src:{args.remote_app}/scripts",
                f"{args.remote_app}/.venv/bin/python",
                f"{args.remote_app}/scripts/open_ended_infra_aware_preliminary_v1.py",
                "--resume",
                "--external-tc",
                "--only-run-id",
                args.run_id,
        ...
        record["class_after_run"] = await _snapshot(runner, endpoints, "class")
    finally:
        ...
        record["restored_qdisc"] = await _snapshot(runner, endpoints, "qdisc")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--remote-app",
        default="/home/super/xiaoming/open_ended_infra_aware_preliminary_v1/app",
    )
    parser.add_argument(
        "--remote-output",
        default="/home/super/xiaoming/open_ended_infra_aware_preliminary_v1/evidence",
    )
    parser.add_argument(
        "--source-evidence",
        default=(
            "/home/super/xiaoming/blind_baseline_semantic_cleanup_v1/"
            "evidence-clean-attempt3"
        ),
    )
    parser.add_argument(
        "--video-sources",
        default="/home/super/xiaoming/calibration_v0/source",
    )
    parser.add_argument(
        "--longbench-samples",
        default=(
